...
def game_loop(args, weather):

    # INFO: Initialize the CARLA simulator
    pygame.init()
    pygame.font.init()
    ...
    try:
        ...
        # INFO: Initialize the benchmark evaluator, parking vehicle and its controller
        ...
        # INFO: Apply the customized envrionment settings and save the weather into logs
        import time
        time.sleep(0.2)
        carla_world.set_weather(weather)
        network_evaluator.save_weather()
        print("Final weather:", carla_world.get_weather())
        logging.info("Save weather success!")
        time.sleep(0.1)
        ...
        while network_evaluator.running == True:
...
def main():
    ...
    # INFO: Set some useful parameters
    ...
    # INFO: Something mainly change the perception - env with the same settings, small domain gap and large domain gaps
    cloudiness = [15.0, 35.0, 45.0]
    sun_azimuth_angle = [0.0, 20.0, 30.0]
    sun_altitude_angle = [75.0, 25.0, 5.0]
    fog_density = [0.0, 2.0, 3.0]

    # INFO: Something mainly change the dynamics
    wetness = [0.0, 20.0, 30.0]

    # INFO: Something change perception and dynamics
    precipitation = [0.0, 30.0, 45.0]
    precipitation_deposits = [0.0, 20.0, 30.0]

    # INFO: We loop each envrionment setting for our benchmarking
    for i, (c, az, alt, fog, wet, pr, prd) in enumerate((list(zip(cloudiness, sun_azimuth_angle, sun_altitude_angle, fog_density, wetness, precipitation, precipitation_deposits))), start=1):
        
        # INFO: Print the current environment settings
        print("pr = ", pr)

        # INFO: Set the weather condition in CARLA
        weather = carla.WeatherParameters(
            cloudiness=float(c),
            precipitation=float(pr),
            precipitation_deposits=float(prd),
            sun_azimuth_angle=float(az),
            sun_altitude_angle=float(alt),
            fog_density=float(fog),
            wetness=float(wet),

        )

        # INFO: Start the benchmarking
        try:
            game_loop(args, weather)
