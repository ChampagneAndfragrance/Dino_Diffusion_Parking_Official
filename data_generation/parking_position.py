...
class EgoPosTown04:
    ...
    def update_eva_goal_y_yaw(self, every_parking_num, parking_idx):
        self.update_yaw_scope()

        self.y = self.y_min
        initial_yaw = self.yaw_to_r if parking_idx < (every_parking_num / 2) else self.yaw_to_l

        if every_parking_num > 1:
            self.yaw_step = (self.yaw_max - self.yaw_min) / (every_parking_num - 1)
