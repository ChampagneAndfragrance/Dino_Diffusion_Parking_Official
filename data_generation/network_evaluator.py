...
class NetworkEvaluator:
    def __init__(self, carla_world, args):
        ...
        # INFO: Evaluation parking spots are different from training - only eva odd slot: 1, 3, 5, ..., 15
        # 2-1, index 15 + 1 = 16
        ...
        # INFO: Start from epoch 0, task (parking spot idx) 0, and parking agent initial pose idx 0
        self._eva_epoch_idx = 0
        self._eva_task_idx = 0
        self._eva_parking_idx = 0

        # INFO: Parking success tolerance
    ...
    def tick(self, clock):
        ...
        # INFO: Check if the goal is reached
        self.eva_check_goal()
    ...
    def start_eva_epoch(self):
        ...
        self._parking_goal_index = 16
        self._parking_goal = parking_position.parking_vehicle_locations_Town04[self._parking_goal_index]
        self._ego_transform_generator.update_eva_goal_y(self._parking_goal.y, self._eva_parking_nums, self._eva_parking_idx)
    ...
    def start_next_parking(self):
        ...
        # INFO: Teporarily save the slot benchmark results if it is done
        ...
        # INFO: Prepare for the next slot
        self.clear_metric_frame()

        # INFO: Move to the next slot
        self._ego_transform = self._ego_transform_generator.get_eva_ego_transform(self._eva_parking_nums, self._eva_parking_idx)
        self._world.player.set_transform(self._ego_transform)
    ...
    def start_next_slot(self):
        ...
        # INFO: When this epoch has been done ...
        if self.is_complete_epoch(self._eva_task_idx):
            ...
            # INFO: If all the epoch has been done, save the data
            if self._eva_epoch_idx >= self._eva_epochs:
                self.save_mean_std_csv()
                self.running = False
                # exit(0)
            # INFO: Otherwise start the next epoch
            else:
                self.start_eva_epoch()
            return

        # INFO: Update the parking goal every other slot when we start the new slot
    ...
    def eva_check_goal(self):
        ...
        # INFO: Check success parking
        if self.check_success_slot(closest_goal, t):
            self.start_next_parking()
            return

        # INFO: Check fail parking
        if self.check_fail_slot(closest_goal, t):
            self.start_next_parking()
            return

    def check_success_slot(self, closest_goal, ego_transform):
        """ This function is to test if the vehicle has been in the success range now """

        # INFO: Is the vehicle in any parking spot pose range?
        ...
        # INFO: If so, the current frame is a frame in one slot
        if x_in_slot and y_in_slot and r_in_slot:
            self._num_frames_in_goal += 1

        # INFO: We need to distinguish if it parks to the desired parking slot even it parks into some parking
        if self._num_frames_in_goal > self._num_frames_in_goal_needed:
            # INFO: If the parking slot it parks into is the desired slot, then add 1 to the target success
            ...
            # INFO: If the parking slot it parks into is NOT the desired slot, then add 1 to the non target success
            else:
    ...
    def check_fail_slot(self, closest_goal, ego_transform):
        """ This function is used to check which failure type the parking is."""

        # INFO: Flag about if the vehicle is near some slot
        x_not_in_slot = \
        ...
        # INFO: If the vehicle is near any parking slot ...
        if x_not_in_slot or y_not_in_slot or r_not_in_slot:
            # INFO: Add 1 to _num_frames_nearby_goal if the parking slot is desired
            if (self._eva_parking_goal[0] == closest_goal[0]) and (self._eva_parking_goal[1] == closest_goal[1]):
                self._num_frames_nearby_goal += 1
            # INFO: Add 1 to _num_frames_nearby_no_goal if the parking slot is not desired
            else:
                self._num_frames_nearby_no_goal += 1

        # INFO: Add 1 to _target_fail_nums if it keeps near but not in the range of the desired slot 
        ...
        # INFO: Add 1 to _no_target_fail_nums if it keeps near but not in the range of the slot other than the desired
